      ...
      <Card className="p-8 shadow-lg rounded-lg bg-white w-full max-w-md">
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            ...
            <Input type="number" id="day" min="1" max="31" value={day === 0 ? "" : day} onChange={(e) => setDay(Number(e.target.value))} />
            {!isDayValid && <p className="text-red-500 text-xs">Must be between 1-31</p>}
          </div>
          <div className="flex flex-col gap-2">
            <Label htmlFor="month">Month</Label>
            <Input type="number" id="month" min="1" max="12" value={month === 0 ? "" : month} onChange={(e) => setMonth(Number(e.target.value))} />
            {!isMonthValid && <p className="text-red-500 text-xs">Must be between 1-12</p>}
          </div>
          <div className="flex flex-col gap-2">
            <Label htmlFor="year">Year</Label>
            <Input type="number" id="year" min="-1000" max="5000" value={year === 0 ? "" : year} onChange={(e) => setYear(Number(e.target.value))} />
            {!isYearValid && <p className="text-red-500 text-xs">Must be between -1000 and 5000, excluding 0</p>}
          </div>
          <Button className="w-full" variant="default" disabled={!isFormValid} onClick={handleSubmit}>
            Submit
          </Button>
                  ...
                  week.map((calendarDay, dayIndex) => (
                    <div
                      ...
                      className={`border rounded p-2 text-center text-sm ${
                        calendarDay === day ? "bg-blue-500 text-white font-bold" : calendarDay ? "bg-gray-200" : "white"
                      }`}
                    >
                      {calendarDay ?? ""}
                    </div>
